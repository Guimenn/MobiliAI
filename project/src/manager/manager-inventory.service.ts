...
export class ManagerInventoryService {
  ...
  async getInventoryStatus(managerId: string) {
  ...
  async updateProductStock(managerId: string, productId: string, newStock: number) {
    // Verificar se o gerente tem uma loja atribuída
    const manager = await this.prisma.user.findUnique({
      where: { id: managerId },
      include: { store: true }
  ...
  async adjustInventory(managerId: string, productId: string, adjustment: number, reason: string) {
    // Verificar se o gerente tem uma loja atribuída
    const manager = await this.prisma.user.findUnique({
      where: { id: managerId },
      include: { store: true }
  ...
  async getInventoryAlerts(managerId: string) {
    // Verificar se o gerente tem uma loja atribuída
    const manager = await this.prisma.user.findUnique({
      where: { id: managerId },
      include: { store: true }
    ...
    const [lowStock, outOfStock, expiringSoon] = await Promise.all([
      this.getLowStockProducts(storeId),
      this.getOutOfStockProducts(storeId),
      this.getExpiringProducts(storeId)
    ]);
    ...
    return {
      ...
      outOfStock,
      expiringSoon,
      totalAlerts: lowStock.length + outOfStock.length + expiringSoon.length
    };
  }
  ...
  private async getExpiringProducts(storeId: string) {
    // Simular produtos próximos do vencimento (se você tiver campo de validade)
    return [];
  }
  ...
  async getInventoryReport(managerId: string, category?: string) {
    // Verificar se o gerente tem uma loja atribuída
    const manager = await this.prisma.user.findUnique({
      where: { id: managerId },
      include: { store: true }
  ...
  async getStockMovement(managerId: string, productId?: string, days = 30) {
    // Verificar se o gerente tem uma loja atribuída
    const manager = await this.prisma.user.findUnique({
      where: { id: managerId },
      include: { store: true }
